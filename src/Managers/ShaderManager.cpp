...
GLuint ShaderManager::build_shader(std::string fragment_loc, std::string vertex_loc) {
	...
	if(!success)
	{
	    glGetShaderInfoLog(VertexShaderID, ERROR_LOG_SIZE, NULL, infoLog);
	    log_error(ERROR_SHADER_COMPILE_VERTEX, infoLog);
	};
	...
	if(!success)
	{
	    glGetShaderInfoLog(FragmentShaderID, ERROR_LOG_SIZE, NULL, infoLog);
	    log_error(ERROR_SHADER_COMPILE_FRAGMENT, infoLog);
	};
