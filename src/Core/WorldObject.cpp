...
void WorldObject::update_verts(std::vector<glm::vec3> &_v,
...
void WorldObject::update(float delta, GLuint programID) {
	...
	if (animated) {
		...
		if (delta_arg >= 0.25f) {
			delta_arg -= 0.25f;
			if (current_model == signed(model.size() - 1))
				current_model = 0;
			else
				current_model++;
		}
...
void WorldObject::display(float delta, GLuint programID) {
	...
	if (draw_bb || !use_model) {
		...
		GLuint matrixAttributePosition = glGetUniformLocation(programID,
				"mat_model_view");
		glm::mat4 _mat_transform = mat_translate * mat_scale;
		glUniformMatrix4fv(matrixAttributePosition, 1, GL_FALSE,
				&_mat_transform[0][0]);
	...
	if (use_model) {
		...
		glDrawElements(GL_TRIANGLES,
				model.at(current_model)->model_indices.size(), GL_UNSIGNED_INT,
				0);
