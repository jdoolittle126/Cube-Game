...
class WorldObject: public GameObject {
	protected:
		float scale_x, scale_y, scale_z, pos_x, pos_y, pos_z, pos_pitch,
				pos_yaw, pos_roll;
		...
		glm::mat4x4 mat_transform, _mat_transform, mat_translate, mat_scale,
				mat_rot;
		...
		void build(float i_size, float i_x, float i_y, float i_z, float i_yaw,
				float i_pitch, float i_roll) {
			scale_x = i_size;
			scale_y = i_size;
			scale_z = i_size;
		...
		void build_debug() {
			...
			glBufferData(GL_ARRAY_BUFFER, bbverts.size() * sizeof(glm::vec3),
					&bbverts[0], GL_STATIC_DRAW);
			glBindBuffer(GL_ARRAY_BUFFER, 0);

			GLuint debug_indices[] = { 0, 1, 2, 2, 3, 0, 4, 5, 6, 6, 7, 4, 0, 3,
					7, 7, 4, 0, 1, 2, 6, 6, 5, 1, 0, 1, 5, 5, 4, 0, 3, 2, 6, 6,
					7, 3
			...
			glBufferData(GL_ELEMENT_ARRAY_BUFFER, 36 * sizeof(unsigned int),
					&debug_indices[0], GL_STATIC_DRAW);
			glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, 0);
		}

	public:
		WorldObject(float i_size, float i_x, float i_y, float i_z, float i_yaw,
				float i_pitch, float i_roll) {
			use_model = false;
		...
		WorldObject(float i_size, float i_x, float i_y, float i_z, float i_yaw,
				float i_pitch, float i_roll, Model *i_model) {
		...
		WorldObject(float i_size, float i_x, float i_y, float i_z, float i_yaw,
				float i_pitch, float i_roll, std::vector<Model*> i_model) {
		...
		void update_verts(std::vector<glm::vec3> &_v,
